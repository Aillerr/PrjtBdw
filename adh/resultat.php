<?php 
	...
	function res_adh() {
		...
		if (isset($_POST['sub_ed']) && !empty($_POST['id_ed'])) {
			...
			//On affiche les informations de temps passage pour l'édition renseignée de l'adhérent
	...
	function form_res() {
		//Affichage des couples courses/éditions
		$p="SELECT e.IdE, e.Annee, c.Nom FROM edition AS e NATURAL JOIN course AS c";
		$tab=traiterRequete($p);
		Array2Table($tab);
		//Formulaire pour savoir quels temps veut-on consulter
